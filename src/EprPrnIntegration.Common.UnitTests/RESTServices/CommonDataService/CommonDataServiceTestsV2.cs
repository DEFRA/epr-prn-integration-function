...
namespace EprPrnIntegration.Common.UnitTests.RESTServices.CommonService
{
    ...
    public class CommonDataServiceTestsV2
    {
        ...
        public async Task GetUpdatedProducers_ShouldReturnCorrectData()
        {
            ...
            // Act
            var result = await commonDataService.GetUpdatedProducersV2(
                fromDate,
                toDate,
                cancellationToken
            );
            ...
            _loggerMock.Verify(
                logger =>
                    logger.Log(
                        ...
                        It.Is<It.IsAnyType>(
                            (v, t) =>
                                $"{v}".ToString().Contains("Getting updated producers list v2.")
                        ),
                        null,
        ...
        public async Task GetUpdatedProducers_ShouldThrowException_WhenApiReturnsError()
        {
            ...
            await Assert.ThrowsAsync<ResponseCodeException>(() =>
                commonDataService.GetUpdatedProducersV2(fromDate, toDate, cancellationToken)
            );
        }
