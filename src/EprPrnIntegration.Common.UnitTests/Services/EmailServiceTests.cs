...
public class EmailServiceTests
{
    ...
    public EmailServiceTests()
    {
        ...
        _messagingConfig = new MessagingConfig
        {
            ...
            NpwdReconcileUpdatedOrganisationsTemplateId =
                "npwdReconcileUpdatedOrganisationTemplateId",
        ...
        _mockLogger.Verify(
            logger =>
                logger.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (state, type) =>
                            ContainsString(state, "GOV UK NOTIFY ERROR. Method: SendEmail")
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
                ),
        ...
        _mockLogger.Verify(
            logger =>
                logger.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>(
                        (state, type) => ContainsString(state, "GOV UK NOTIFY ERROR")
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
                ),
                ...
                logger.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (state, type) =>
                            ContainsString(
                                state,
                                $"Failed to send email to {_messagingConfig.NpwdEmail} using template ID {_messagingConfig.NpwdValidationErrorsTemplateId}"
                            )
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
            ...
            .Setup(client =>
                client.SendEmail(
                    ...
                    It.Is<string>(template =>
                        template == _messagingConfig.NpwdReconcileIssuedPrnsTemplateId
                    ),
                    It.Is<Dictionary<string, object>>(parameters =>
        ...
        _mockLogger.Verify(
            logger =>
                logger.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>(
                        (state, type) => ContainsString(state, "Reconciliation email sent to NPWD")
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
                ),
        ...
        _mockNotificationClient
            .Setup(client =>
                client.SendEmail(
                    ...
                    It.Is<Dictionary<string, object>>(parameters =>
                        parameters.ContainsKey("date") && parameters.ContainsKey("csvData")
                    ),
                    null,
                    null,
                    null
                ...
                logger.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (state, type) =>
                            ContainsString(
                                state,
                                $"Failed to send email to {_messagingConfig.NpwdEmail} using template ID {_messagingConfig.NpwdReconcileUpdatedPrnsTemplateId}"
                            )
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
    ...
    public void SendUpdatedOrganisationsReconciliationEmailToNpwd_SendsEmailWithAttachment_Successfully()
    {
        ...
        _mockNotificationClient
            .Setup(client =>
                client.SendEmail(
                    It.Is<string>(email => email == _messagingConfig.NpwdEmail),
                    It.Is<string>(template =>
                        template == _messagingConfig.NpwdReconcileUpdatedOrganisationsTemplateId
                    ),
                    It.Is<Dictionary<string, object>>(parameters =>
        ...
        // Act
        _emailService.SendUpdatedOrganisationsReconciliationEmailToNpwd(
            new DateTime(2025, 12, 1),
            1,
            "Sample CSV Content"
        );
                ...
                logger.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (state, type) =>
                            ContainsString(
                                state,
                                "Updated organisations reconciliation email sent to NPWD"
                            )
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
                ...
                logger.Log(
                    ...
                    It.Is<It.IsAnyType>(
                        (state, type) =>
                            ContainsString(
                                state,
                                $"Failed to send email to {_messagingConfig.NpwdEmail} using template ID {_messagingConfig.NpwdReconcileUpdatedOrganisationsTemplateId}"
                            )
                    ),
                    It.IsAny<Exception>(),
                    It.Is<Func<It.IsAnyType, Exception?, string>>((state, ex) => true)
    ...
    public void SendIssuedPrnsReconciliationEmailToNpwd_ThrowsError_WhenRequiredParametersNotPassed()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentException>(() =>
            _emailService.SendIssuedPrnsReconciliationEmailToNpwd(reportDate, 0, csvData)
        );
    }
    ...
    public void SendUpdatedPrnsReconciliationEmailToNpwd_ThrowsError_WhenRequiredParametersNotPassed()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentException>(() =>
            _emailService.SendUpdatedPrnsReconciliationEmailToNpwd(reportDate, csvData, 0)
        );
    }
    ...
    public void SendUpdatedOrganisationsReconciliationEmailToNpwd_ThrowsError_WhenRequiredParametersNotPassed()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentException>(() =>
            _emailService.SendUpdatedOrganisationsReconciliationEmailToNpwd(reportDate, 0, csvData)
        );
    }
