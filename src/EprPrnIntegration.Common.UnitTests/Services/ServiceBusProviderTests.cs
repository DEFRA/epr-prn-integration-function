...
public class ServiceBusProviderTests
{
    ...
    public async Task SendFetchedNpwdPrnsToQueue_SuccessfullyPushMessage()
    {
        ...
        _serviceBusSenderMock
            .Setup(sender => sender.CreateMessageBatchAsync(default))
            .ReturnsAsync(messageBatch);
        _serviceBusClientMock
        ...
        _serviceBusSenderMock.Verify(
            sender => sender.SendMessagesAsync(It.IsAny<ServiceBusMessageBatch>(), default),
            Times.Once
        );
        _serviceBusSenderMock.Verify(r => r.DisposeAsync(), Times.Once);
        _loggerMock.VerifyLog(
            l => l.LogInformation(It.Is<string>(msg => msg.Contains("SendFetchedNpwdPrnsToQueue"))),
            Times.Exactly(2)
        );
    }
    ...
    public async Task SendFetchedPrnsToQueue_SendBatchAndCreateNewAndAddMessage()
    {
        ...
        _serviceBusSenderMock
            .SetupSequence(sender => sender.CreateMessageBatchAsync(default))
        ...
        _serviceBusSenderMock.Verify(
            sender => sender.CreateMessageBatchAsync(default),
            Times.Exactly(2)
        );
    ...
    public async Task SendFetchedPrnsToQueue_ShouldThrowError_WHenClientFails()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() =>
            _serviceBusProvider.SendFetchedNpwdPrnsToQueue(npwdPrns)
        );

        // Assert
        _serviceBusSenderMock.Verify(r => r.DisposeAsync(), Times.Once);
        _loggerMock.VerifyLog(
            l => l.LogError(It.IsAny<Exception>(), It.Is<string>(s => !string.IsNullOrEmpty(s))),
            Times.Once
        );
    }
        ...
        _loggerMock.VerifyLog(
            l =>
                l.LogError(
                    It.Is<string>(s =>
                        s.Contains(
                            "SendFetchedNpwdPrnsToQueue failed to add message on Queue with exception"
                        )
                    )
                ),
            Times.Once
        ...
        _loggerMock.VerifyLog(
            logger =>
                logger.LogInformation(
                    It.Is<string>(s =>
                        s.Contains(
                            "SendDeltaSyncExecutionToQueue - A message has been published to the queue"
                        )
                    )
                ),
            Times.Once
    ...
    public async Task SendDeltaSyncExecutionToQueue_ThrowsException_LogsError()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() =>
            _serviceBusProvider.SendDeltaSyncExecutionToQueue(deltaSyncExecution)
        );
        _loggerMock.VerifyLog(
            logger =>
                logger.LogError(
                    It.Is<string>(s =>
                        s.Contains(
                            "SendDeltaSyncExecutionToQueue failed to add message on Queue with exception"
                        )
                    )
                ),
            Times.Once
    ...
    public async Task ReceiveDeltaSyncExecutionFromQueue_NoMessages_ReturnsNull()
    {
        ...
        _serviceBusReceiverMock
            .Setup(receiver =>
                receiver.ReceiveMessageAsync(It.IsAny<TimeSpan>(), It.IsAny<CancellationToken>())
            )
            .ReturnsAsync((ServiceBusReceivedMessage?)null);

        // Act
        var result = await _serviceBusProvider.GetDeltaSyncExecutionFromQueue(
            NpwdDeltaSyncType.UpdatedProducers
        );
        ...
        _loggerMock.VerifyLog(
            logger =>
                logger.LogInformation(
                    It.Is<string>(s => s.Contains("No message received from the queue"))
                ),
            Times.Once
        );
    }
    ...
    {
        // Arrange
        var sync1 = new DeltaSyncExecution()
        {
            LastSyncDateTime = DateTimeHelper.Parse("2024-10-08"),
            SyncType = NpwdDeltaSyncType.FetchNpwdIssuedPrns,
        };
        var sync2 = new DeltaSyncExecution()
        {
            LastSyncDateTime = DateTimeHelper.Parse("2024-10-09"),
            SyncType = NpwdDeltaSyncType.FetchNpwdIssuedPrns,
        };
        var sync3 = new DeltaSyncExecution()
        {
            LastSyncDateTime = DateTimeHelper.Parse("2024-10-10"),
            SyncType = NpwdDeltaSyncType.FetchNpwdIssuedPrns,
        };

        var message1 = ServiceBusModelFactory.ServiceBusReceivedMessage(
            new BinaryData(sync1),
            sequenceNumber: 1
        );
        var message2 = ServiceBusModelFactory.ServiceBusReceivedMessage(
            new BinaryData(sync2),
            sequenceNumber: 2
        );
        var latestMessage = ServiceBusModelFactory.ServiceBusReceivedMessage(
            new BinaryData(sync3),
            sequenceNumber: 3
        );

        _serviceBusReceiverMock
            .Setup(r =>
                r.ReceiveMessagesAsync(
                    It.IsAny<int>(),
                    It.IsAny<TimeSpan>(),
                    It.IsAny<CancellationToken>()
                )
        ...
        // Act
        var result = await _serviceBusProvider.GetDeltaSyncExecutionFromQueue(
            NpwdDeltaSyncType.FetchNpwdIssuedPrns
        );
        ...
        _serviceBusReceiverMock.Verify(
            ...
            Times.Exactly(completeMessageCalls)
        );
    ...
    {
        // Arrange
        var invalidMessage = ServiceBusModelFactory.ServiceBusReceivedMessage(
            new BinaryData(JsonSerializer.Serialize("invalid message"))
        );
        ...
        _loggerMock.VerifyLog(
            logger =>
                logger.LogError(
                    It.Is<string>(s => s.Contains("Failed to deserialize message body:"))
                ),
            Times.Once
        );
    }
    ...
    public async Task ReceiveDeltaSyncExecutionFromQueue_ExceptionThrown_LogsErrorAndRethrows()
    {
        ...
        _serviceBusReceiverMock
            .Setup(receiver =>
                receiver.ReceiveMessagesAsync(It.IsAny<int>(), It.IsAny<TimeSpan>(), default)
            )
            .ThrowsAsync(new Exception("Service bus connection error"));

        // Act & Assert
        var exception = await Assert.ThrowsAsync<Exception>(() =>
            _serviceBusProvider.GetDeltaSyncExecutionFromQueue(NpwdDeltaSyncType.UpdatedProducers)
        );
        ...
        _loggerMock.VerifyLog(
            logger =>
                logger.LogError(
                    It.Is<string>(s =>
                        s.Contains("GetDeltaSyncExecutionFromQueue failed with exception")
                    )
                ),
            Times.Once
        );
    ...
    public void GetDeltaSyncQueueName_ReturnsCorrectQueueName()
    {
        ...
        // Act
        var updatedProducersQueue = _serviceBusProvider
            .GetType()
            .GetMethod(
                "GetDeltaSyncQueueName",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance
            )!
            .Invoke(_serviceBusProvider, [NpwdDeltaSyncType.UpdatedProducers]);
        var updatePrnsQueue = _serviceBusProvider
            .GetType()
            .GetMethod(
                "GetDeltaSyncQueueName",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance
        ...
        var fetchNpwdIssuedPrnsQueue = _serviceBusProvider
            .GetType()
            .GetMethod(
                "GetDeltaSyncQueueName",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance
    ...
    public async Task SendMessageToErrorQueue_Success()
    {
        ...
        var receivedMessage = ServiceBusModelFactory.ServiceBusReceivedMessage(
            body: new BinaryData(
                Newtonsoft.Json.JsonConvert.SerializeObject(
                    _fixture.Create<Common.Models.Npwd.Evidence>()
                )
            ),
        ...
        // Assert
        _serviceBusSenderMock.Verify(
            r => r.SendMessageAsync(It.IsAny<ServiceBusMessage>(), It.IsAny<CancellationToken>()),
            Times.Once
        );
        _loggerMock.VerifyLog(
            l => l.LogInformation(It.Is<string>(msg => msg.Contains("Message with EvidenceNo"))),
            Times.Once
        );
    }
            ...
            sender =>
                sender.SendMessageAsync(
                    It.Is<ServiceBusMessage>(msg =>
                        ...
                        && msg.ApplicationProperties.SequenceEqual(
    ...
    public async Task ProcessFetchedPrns_ShouldNotCallHandlerAndBreakLoop_WhenNoMessagesFoundInQueue()
    {
        ...
        _serviceBusClientMock
            .Setup(client =>
                client.CreateReceiver(It.IsAny<string>(), It.IsAny<ServiceBusReceiverOptions>())
            )
            .Returns(_serviceBusReceiverMock.Object);
        _serviceBusReceiverMock
            .Setup(receiver =>
    ...
    public async Task ProcessFetchedPrns_ShouldCallHandlerAndBreakLoop_WhenMessagesFoundInQueue()
    {
        ...
        var messages = npwdPrns
            .Select(prn =>
                ServiceBusModelFactory.ServiceBusReceivedMessage(
                    new BinaryData(JsonSerializer.Serialize(npwdPrns))
                )
            )
            .ToList();
        ...
        _loggerMock.Verify(
            logger =>
                logger.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>(
                        (v, t) => v.ToString()!.Contains("Failed to process message with id:")
                    ),
                    It.IsAny<Exception>(),
        ...
        _loggerMock.Verify(
            logger =>
                logger.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>(
                        (v, t) => v.ToString()!.Contains("Failed to receive messages from queue:")
                    ),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()
                ),
