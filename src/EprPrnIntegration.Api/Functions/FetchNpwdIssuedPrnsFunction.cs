...
namespace EprPrnIntegration.Api.Functions
{
    public class FetchNpwdIssuedPrnsFunction
    {
        ...
        public FetchNpwdIssuedPrnsFunction(
            ILogger<FetchNpwdIssuedPrnsFunction> logger,
            ICoreServices core,
            IMessagingServices messaging,
            IValidator<NpwdPrn> validator,
        ...
        public async Task Run([TimerTrigger("%FetchNpwdIssuedPrns:Schedule%")] TimerInfo timerInfo)
        {
            ...
            _logger.LogInformation(
                "FetchNpwdIssuedPrnsFunction function started at: {DateTime}",
                DateTime.UtcNow
            );

            var deltaRun = await _utilities.GetDeltaSyncExecution(
                NpwdDeltaSyncType.FetchNpwdIssuedPrns
            );
            var now = DateTime.UtcNow;
            var toDate = _utilities.OffsetDateTimeWithLag(
                now,
                _configuration["FetchNpwdPrnsPollingLagSeconds"]
            );

            if (!toDate.Equals(now))
            {
                _logger.LogInformation(
                    "Upper date range {Now} rolled back to {ToDate}",
                    now,
                    toDate
                );
            }
            ...
            _logger.LogInformation(
                "Fetching From: {FromDate} and To {ToDate} dates for this execution",
                deltaRun.LastSyncDateTime,
                toDate
            );
            ...
            var validationFailedPrns = await _messaging.ServiceBusProvider.ProcessFetchedPrns(
                ProcessFetchedPrn
            );
            ...
            _logger.LogInformation(
                "FetchNpwdIssuedPrnsFunction function Completed at: {DateTime}",
                DateTime.UtcNow
            );
        }

        internal async Task<Dictionary<string, string>?> ProcessFetchedPrn(
        ...
        )
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Validating message with Id: {MessageId}",
                    message.MessageId
                );
                ...
                if (validationResult.IsValid)
                {
                    ...
                    {
                        _logger.LogInformation(
                            "Validation passed for message Id: {MessageId}. Processing the PRN.",
                            message.MessageId
                        );
                        ...
                        await _core.PrnService.SaveNpwdPrn(request);

                        _logger.LogInformation(
                            "Successfully saved PRN details for EvidenceNo: {EvidenceNo}",
                            request.EvidenceNo
                        );
                    ...
                    catch (Exception ex)
                    {
                        _logger.LogError(
                            ex,
                            "Error processing message Id: {MessageId}. Adding it back to the queue.",
                            message.MessageId
                        );

                        await _messaging.ServiceBusProvider.SendMessageToErrorQueue(
                            message,
                            evidenceNo
                        );
                    }
                ...
                {
                    _logger.LogWarning(
                        "Validation failed for message Id: {MessageId}. Sending to error queue.",
                        message.MessageId
                    );
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(
                    ex,
                    "Unexpected error while processing message Id: {MessageId}.",
                    message.MessageId
                );
        ...
        private static Dictionary<string, string> CreateCustomEvent(
            NpwdPrn? npwdPrn,
            string errorMessage = ""
        )
        {
            Dictionary<string, string> eventData = new()
            {
                { CustomEventFields.PrnNumber, npwdPrn?.EvidenceNo ?? "No PRN Number" },
                {
                    ...
                    npwdPrn?.EvidenceStatusCode ?? "Blank Incoming Status"
                },
                { CustomEventFields.Date, DateTime.UtcNow.ToString() },
                {
                    CustomEventFields.OrganisationName,
                    npwdPrn?.IssuedToOrgName ?? "Blank Organisation Name"
                },
            };
        ...
        private async Task SendEmailToProducers(
            ServiceBusReceivedMessage message,
            NpwdPrn? messageContent,
            SaveNpwdPrnDetailsRequest request
        )
        {
            try
            {
                ...
                var producerEmails =
                    await _core.OrganisationService.GetPersonEmailsAsync(
                        messageContent!.IssuedToEPRId!,
                        messageContent.IssuedToEntityTypeCode!,
                ...
                foreach (var producer in producerEmails)
                {
                    var producerEmail = new ProducerEmail
                    {
                        ...
                        IsExporter = NpwdPrnToSavePrnDetailsRequestMapper.IsExport(
                            request.EvidenceNo!
                        ),
                    };
                ...
                {
                    _messaging.EmailService.SendCancelledPrnsNotificationEmails(
                        producers,
                        messageContent!.IssuedToEPRId!
                    );
                }
                ...
                {
                    _messaging.EmailService.SendEmailsToProducers(
                        producers,
                        messageContent!.IssuedToEPRId!
                    );
                }
                ...
                _logger.LogInformation(
                    "Successfully processed and sent emails for message Id: {MessageId}",
                    message.MessageId
                );
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ...
                    messageContent?.EvidenceNo,
                    messageContent?.IssuedToEPRId
                );
            }
        }

        private void SendErrorFetchedPrnEmail(
            List<Dictionary<string, string>> validatedErrorMessages
        )
        {
            try
            {
                if (validatedErrorMessages.Count > 0)
                {
                    ...
                    {
                        {
                            CustomEventFields.PrnNumber,
                            validatedErrorMessages
                                .Select(kv =>
                                    kv.GetValueOrDefault(
                                        CustomEventFields.PrnNumber,
                                        "No PRN Number"
                                    )
                                )
                        ...
                        {
                            CustomEventFields.ErrorComments,
                            validatedErrorMessages
                                .Select(kv =>
                                    kv.GetValueOrDefault(
                                        CustomEventFields.ErrorComments,
        ...
        private string GetFilterToFetchPrns(DeltaSyncExecution deltaRun, DateTime toDate)
        {
            ...
            if (
                ...
                && DateTime.TryParseExact(
                    _configuration["DefaultLastRunDate"],
                    "yyyy-MM-dd",
                    CultureInfo.InvariantCulture,
                    DateTimeStyles.None,
                    out DateTime defaultLastRunDate
                )
                && deltaRun.LastSyncDateTime > defaultLastRunDate
            )
            {
                filter =
                    $@"{filter} and ((StatusDate ge {deltaRun.LastSyncDateTime.ToUniversalTime():O} and StatusDate lt {toDate.ToUniversalTime():O}) or (ModifiedOn ge {deltaRun.LastSyncDateTime.ToUniversalTime():O} and ModifiedOn lt {toDate.ToUniversalTime():O}))";
            }
        ...
        private async Task<List<NpwdPrn>> FetchPrns(string filter)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation(
                        "Total: {Count} fetched from Npwd with filter {Filter}",
                        npwdIssuedPrns!.Count,
                        filter
                    );
                }
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(
                    ex,
                    "Failed Get Prns from npwd for filter {Filter} with exception {Ex}",
                    filter,
                    ex
                );
                ...
                {
                    _messaging.EmailService.SendErrorEmailToNpwd(
                        $"Failed to fetch issued PRNs. error code {ex.StatusCode} and raw response body: {ex.Message}"
                    );
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed Get Prns method for filter {Filter} with exception {Ex}",
                    filter,
                    ex.Message
                );
        ...
        private async Task PushPrnsToInputQueue(List<NpwdPrn> npwdIssuedPrns)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed pushing issued prns in message queue with exception: {Ex}",
                    ex
                );
