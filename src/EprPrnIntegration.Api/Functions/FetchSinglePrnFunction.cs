...
)
{
    ...
    )
    {
        ...
        try
        {
            var fetchSinglePrnRequest =
                await JsonSerializer.DeserializeAsync<FetchSinglePrnRequest>(req.Body);
            ...
            if (fetchedMissingPrn is null)
            {
                return new NotFoundObjectResult(
                    $"{fetchSinglePrnRequest.PrnNumber} is not found in NPWD system."
                );
            }
            ...
            return new OkObjectResult(
                $"{fetchSinglePrnRequest.PrnNumber} is produced to the queue to be processed."
            );
        }
        catch (Exception ex)
        {
    ...
    {
        var filter =
            $"(EvidenceStatusCode eq 'EV-CANCEL' or EvidenceStatusCode eq 'EV-AWACCEP' or EvidenceStatusCode eq 'EV-AWACCEP-EPR') AND evidenceNo eq '{prnNumber}'";
