...
public class RrepwOAuthMiddleware : DelegatingHandler
{
    private readonly RrepwIntegrationConfiguration _npwdIntegrationConfig;
    private readonly IConfidentialClientApplication _confidentialClientApplication;
    private readonly ILogger<RrepwOAuthMiddleware> _logger;
    private string? _accessToken;

    public RrepwOAuthMiddleware(IOptions<RrepwIntegrationConfiguration> npwdConfig, ILogger<RrepwOAuthMiddleware> logger)
    {
        _logger = logger;
        ...
        if (String.IsNullOrEmpty(_npwdIntegrationConfig.ClientId))
        {
            ...
            _logger.LogWarning("Skipping RREPW auth middleware because no client id configured.");
            return;
        }
        ...
        {
            _logger.LogError(ex, "RrepwOAuthMiddleware failed");
            throw new TypeInitializationException(fullTypeName: $"Failed to initialize {nameof(RrepwOAuthMiddleware)}", innerException: ex);
        }
    }
