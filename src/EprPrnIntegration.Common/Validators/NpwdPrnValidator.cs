...
namespace EprPrnIntegration.Common.Validators
{
    public class NpwdPrnValidator : AbstractValidator<NpwdPrn>
    {
        ...
        public NpwdPrnValidator(IOrganisationService organisationService)
        {
            ...
            RuleFor(prn => prn.IssuedToEPRId)
                .MustAsync(
                    async (prn, id, cancellation) => await BeValidIssuedToEPRIdAsync(prn, id)
                )
                .WithMessage(
                    "IssuedToEPRId must match an existing PEPR organisation or compliance scheme"
                );
        ...
        private async Task<bool> BeValidIssuedToEPRIdAsync(NpwdPrn npwdPrn, string? eprId)
        {
            ...
            if (Guid.TryParse(npwdPrn.IssuedToEPRId, out _))
            {
                try
                {
                    return await _organisationService.DoesProducerOrComplianceSchemeExistAsync(
                        npwdPrn.IssuedToEPRId,
                        npwdPrn.IssuedToEntityTypeCode ?? string.Empty,
