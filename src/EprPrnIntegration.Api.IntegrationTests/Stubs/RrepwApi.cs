...
{
    public async Task HasPrnUpdates(string[] prnNumbers, string? cursor = null, string? nextCursor = null)
    {
        var items = prnNumbers.Select(prnNumber => new
        {
            ...
            builder.WithRequest(request =>
                {
                    ...
                    if (cursor != null)
                    {
                        req.WithParams(() => new List<ParamModel>
                        {
                            new()
                            {
                                Name = "cursor",
                                Matchers = new[]
                                {
                                    new MatcherModel { Name = "ExactMatcher", Pattern = cursor }
                                }
                            }
                        });
                    }
