...
public abstract class IntegrationTestBase : IAsyncLifetime
{
    ...
    {
        return await FunctionExecutionContext.GetLastUpdate(functionName) ?? DateTime.MinValue;
    }

    protected static async Task LastUpdateShouldHaveChanged(DateTime before, string functionName)
    {
        var after = await FunctionExecutionContext.GetLastUpdate(functionName) ?? DateTime.MinValue;
